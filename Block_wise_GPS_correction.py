...
def update_single_coordinate_gdal(patch_dict):
	...
	ds = gdal.Warp(out_path, ds, outputBounds = [u_l[0], l_r[1], l_r[0], u_l[1]])
	ds = None
	...
	with open(settings.coordinates_file) as f:
		...
		for l in lines.split('\n'):
			...
			if settings.PATCH_SIZE_GPS[0] == -1:
				...
				settings.Height_Scale = (settings.PATCH_SIZE_GPS[0]/settings.Height_Scale[0]+settings.PATCH_SIZE_GPS[1]/settings.Height_Scale[1])/2
				settings.LID_SIZE_AT_SCALE = (settings.LID_SIZE[0]*settings.SCALE/settings.Height_Scale,settings.LID_SIZE[1]*settings.SCALE/settings.Height_Scale)
...
class Global_Optimizer:
	...
	def transformation_diff_only_least_squares_with_lids(self,corrected_patches):
		...
		GPS_coef_x = 1/settings.GPS_ERROR_STD[0]
		GPS_coef_y = 1/settings.GPS_ERROR_STD[1]
	...
	def get_patches_with_possible_lids(self):
		...
		for g in self.groups:
			for p in g.patches:
				...
				for l in lids:
					if (p.gps.is_coord_inside(lids[l]) or p.gps.is_point_near(lids[l],settings.lid_search_surrounding_patch_number*settings.PATCH_SIZE_GPS[0])) and (p not in [ptch[0] for ptch in lid_patches]):
